...
def test_csv_import_with_errors():
    ...
    with app.app_context():
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
            ...
            # 写入另一个有效数据
            writer.writerow(['设备故障3', '设备C在设计阶段存在问题', '设备C', 'design', 'medium', '王五', '2023-10-03'])
            # 写入包含特殊字符过多的数据（超过50%特殊字符）
            special_chars_text = ''.join(['@' if i % 2 == 0 else '测' for i in range(100)])
            writer.writerow(['设备故障4', f'设备D问题{special_chars_text}', '设备D', 'development', 'high', '赵六', '2023-10-04'])
            # 写入空标题和描述的数据
            writer.writerow(['', '', '设备E', 'maintenance', 'critical', '孙七', '2023-10-05'])
            # 写入有效数据
        ...
        try:
            ...
            # 获取最近导入的问题
            recent_problems = Problem.query.order_by(Problem.id.desc()).limit(result['importedCount']).all()
            print(f"最近导入的问题数: {len(recent_problems)}")
            ...
            if result.get('failedRecords'):
                ...
                with open(failed_file_path, 'r', encoding='utf-8') as f:
                    ...
                    for i, row in enumerate(failed_rows):
                        print(f"  第{i+1}行: {row['title'][:20]}... - 错误信息: {row['error_info'][:50]}...")
